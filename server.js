...
const VERSION = "0.2.0";
const HOST = "::";
const PORT = 8765; // Change it to your server port.
...
function isIPv6(ip) {
  return Boolean(ip && ip.includes(":") && !isIPv4(ip));
}
...
async function resolvePublicIpv6FromHostname(hostname) {
  ...
  try {
    const v6s = await dns.promises.resolve6(String(hostname));
    return v6s.find(isPublicIPv6) || null;
  } catch {
    return null;
  }
}
  ...
  wss.on("connection", (ws, req) => {
    void (async () => {
      ...
      const c = {
        ...
        ipv4: picked.ipv4,
        ipv6: picked.ipv6,
        hasIpv4: picked.hasV4,
        hasIpv6: picked.hasV6,
        reportedExtra: null,
        username: null,
        clientType: "native",
        rl: [],
        snowflake: mkClientId(),
        uuid: mkUUID(),
        lastMsg: 0,
      };
      clients.set(ws, c);
...
case "keepalive":
  ...
  return sendJson(ws,
    typeof t === "number"
      ? { cmd: "pong", t, server: Date.now() }
      : { cmd: "pong", server: Date.now() }
  );
...
function httpHandler(req, res) {
  ...
  if (req.url.startsWith("/status")) {
    const connectedIps = [...clients.values()].map(c => ({
      ip: c.ip || "unknown",
      username: c.username || "",
      id: c.snowflake,
      uuid: c.uuid,
    ...
    const data = JSON.stringify({
      ...
      connectedIps,
      motd: motd.enabled ? motd.text : "",
      motdEnabled: motd.enabled,
...
<head>
  ...
  <style>
    *{box-sizing:border-box;margin:0;padding:0}
    body{background:#0d1117;color:#e6edf3;font-family:monospace;padding:32px;min-height:100vh}
...
<body>
  <h1>⚡ CloudLink V4 Universal Server</h1>
  <span class="badge">● ONLINE</span>

  <div class="card">
    <div class="row"><span class="label">Version</span><span class="value" id="version">…</span></div>
    <div class="row"><span class="label">PID</span><span class="value" id="pid">…</span></div>
    <div class="row"><span class="label">Uptime</span><span class="value" id="uptime">…</span></div>
    <div class="row"><span class="label">TLS</span><span class="value" id="tls">…</span></div>
    <div class="row"><span class="label">Clients</span><span class="value" id="clients">…</span></div>
    <div class="row"><span class="label">Users</span><span class="value" id="users">…</span></div>
    <div class="row"><span class="label">Global vars</span><span class="value" id="gvars">…</span></div>
    <div class="row"><span class="label">MOTD</span><span class="value" id="motd" style="color:#e3b341;">…</span></div>
    <div class="row"><span class="label">Supports</span><span class="value">TurboWarp · PenguinMod · Scratch · All forks</span></div>
    <div class="row"><span class="label">Endpoint</span><span class="value wss" id="endpoint">…</span></div>
  </div>

  <div class="card">
    <div class="label">Users online: <span id="ucount" style="color:#3fb950">0</span></div>
    <div class="users" id="ulist"><span class="empty pulse">Loading…</span></div>
  </div>

  <div class="card">
    <div class="label" style="margin-bottom:8px">Connected IPs:</div>
    <div class="users" id="ips"><span class="empty pulse">Loading…</span></div>
  </div>

  <div class="card">
    <div class="label" style="margin-bottom:8px">Connect from any CloudLink extension:</div>
    ...
    async function refresh() {
      ...
      if (ulist) {
        ulist.innerHTML = list.length
          ...
          : '<span class="empty pulse">No users connected</span>';
      }
      ...
      if (ipsEl) {
        ipsEl.innerHTML = ips.length
          ...
          : '<span class="empty pulse">No IPs connected</span>';
      }
    }
...
async function start() {
  ...
  try {
    ...
    console.log(`  ${scheme}://cloudlinkv4.duckdns.org:8765`); // Change it to your domain
